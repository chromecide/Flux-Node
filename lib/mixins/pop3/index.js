...
var mixinFunctions = {
	...
	pop3_addAccount: function(name, host, port, username, password, poll){
		...
		if(self.debug) console.log('account added');
	},
	...
	pop3_doPoll: function(){ //TODO: Need to move the poll count checks out to the main object, we are getting a conflict when trying to create multiple POP3Clients
		...
		if(self.debug) console.log('CHECKING ACCOUNTS');
		if(self.pop3_Accounts){
			if(self.debug) console.log('STARTING');
			for(var accountName in self.pop3_Accounts){
				var account = self.pop3_Accounts[accountName];
				if(self.debug) console.log(account);
				if(account.poll===true){
					if(self.debug) console.log('Processing: '+accountName);
					...
					client.on("invalid-state", function(cmd) {
						if(self.debug) console.log('invalid state');
						account.status = 'starting'; //not sure if this is a ref yet so just to be sure
					...
					client.on("locked", function(cmd) {
						if(self.debug) console.log('locked');
						self.pop3_onLocked(account, cmd);
					});
					
					client.on("login", function(status, rawdata) {
						if(self.debug) console.log('logged in');
	...
	pop3_onError: function(account, err){
        ...
        if (err.errno === 111) if(self.debug) console.log("Unable to connect to server");
        else if(self.debug) console.log("Server error occurred");
	...
	pop3_onConnect: function(account, client){
		var self = this;
		if(self.debug) console.log("CONNECT success");
		self.emit('POP3.Connected', account);
        if(account.polling===true){
        	if(self.debug) console.log('Authorising: '+account.username+':'+account.password);
        	account.client.login(account.username, account.password);
        }
	},
	pop3_onInvalidState: function(account, cmd){
		var self = this;
		if(self.debug) console.log("Invalid state. You tried calling " + cmd);
	...
	pop3_onLocked: function(account, cmd){
		var self = this;
		if(self.debug) console.log("Current command has not finished yet. You tried calling " + cmd);
		self.emit('POP3.InvalidState', {
			account: account.client.data,
			cmd: cmd
		});  
	...
	pop3_onLogin: function(account, status, rawData){
		var self = this;
		if (status) {
			if(self.debug) console.log("LOGIN/PASS success");
			...
			if(self.debug) console.log("LOGIN/PASS failed");
	...
	pop3_onCapa: function(account, status, data, rawdata){
		var self = this;
		if (status) {
			if(self.debug) console.log("CAPA success");
			if(self.debug) console.log("	Parsed data: " + util.inspect(data));
			if(account.polling===true){
				account.client.noop();	
			}
		} else {
			if(self.debug) console.log("CAPA failed");
			//account.client.quit();
			account.client.noop();
		}
	},
	pop3_onNoop: function(account, status, rawdata) {
		var self = this;
		if (status) {
			if(self.debug) console.log("NOOP success");
			if(account.polling===true){
				account.client.stat();	
			}
		} else {
			if(self.debug) console.log("NOOP failed");
			account.client.quit();
		}
	...
	pop3_onStat: function(account, status, data, rawdata) {
		var self = this;
		if (status === true) {
			if(self.debug) console.log("STAT success");
			if(self.debug) console.log("	Parsed data: " + util.inspect(data));
			if(account.polling){
				account.client.list();	
			}
		} else {
			if(self.debug) console.log("STAT failed");
			account.client.quit();
		}
	},
	pop3_onList: function(account, status, msgcount, msgnumber, data, rawdata) {
		...
		var self = this;
		if (status === false) {
			if(self.debug) console.log("LIST failed");
			...
			if(self.debug) console.log("LIST success with " + msgcount + " element(s)");
	...
	pop3_onUIDL: function(account, status, msgnumber, data, rawdata){
		...
		if(account.polling){
			...
			if(self.debug) console.log('Retrieving '+account.poll_currentmsg+' of '+account.poll_msgcount);
	...
	pop3_onRetrieve: function(account, status, msgnumber, data, rawdata) {
		var self = this;
		if (status === true) {
			if(self.debug) console.log("RETR success for msgnumber " + msgnumber);
			...
			if(self.debug) console.log("RETR failed for msgnumber " + msgnumber);
	...
	pop3_onDelete: function(account, status, msgnumber, data, rawdata) {
		var self = this;
		if (status === true) {
			if(self.debug) console.log("DELE success for msgnumber " + msgnumber);
			...
			if(self.debug) console.log("DELE failed for msgnumber " + msgnumber);
	...
	pop3_onQuit: function(account, status, rawdata){
		var self = this;
		if (status === true){
			if(self.debug) console.log("QUIT success");
...
if (typeof define === 'function' && define.amd) {
	...
	if(self.debug) console.log('Browser Support is not supplied for the POP3 Mixin.');
} else {
