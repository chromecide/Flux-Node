...
var mixinFunctions = {
	...
	pop3_addAccount: function(name, host, port, username, password, poll){
		...
		itemMailParser.on('end', function(mailObj){
			self.pop3_onMessageParsed(self.pop3_Accounts[name], mailObj);
		});
			...
			for(var accountName in self.pop3_Accounts){
				...
				if(account.poll===true){
					...
					client.on("error", function(err) {
						if(self.pop3_Settings.debug) console.log('here');
	...
	pop3_onList: function(account, status, msgcount, msgnumber, data, rawdata) {
		...
		if (status === false) {
			...
			if (msgcount > 0){
				account.poll_msgcount = msgcount;
	...
	pop3_onRetrieve: function(account, status, msgnumber, data, rawdata) {
		...
		if (status === true) {
			...
			if(account.polling && account.poll_currentmsg<=account.poll_msgcount){
				...
				acount.client.quit();//we're done with this account
			}
		} else {
	...
	pop3_onQuit: function(account, status, rawdata){
		...
		if (status === true){
			...
			self.emit('POP3.ClientQuit', {
				account:account.client.data,
				status: status,
				rawdata:rawdata
			});
		}else{
			self.emit('POP3.ClientQuitFailed', {
				account:account.client.data,
				status: status,
				rawdata:rawdata
			});
