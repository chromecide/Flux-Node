...
if (typeof define === 'function' && define.amd) {
	var browserMixinFunctions = {
		...
		init: function(cfg, callback){
			...
			//if the node has flux-ui loaded
			if(thisNode.FluxUI_getWorkspace){
	...
	var nodeMixinFunctions = {
		...
		init: function(cfg, callback){
			...
			thisNode.registerMiddleware('TunnelManager.Message', function(tunnel, topic, message, next){
				...
				if(!tunnel.isLoggedIn){
					...
					if(topic=='GroupSecurity.Login'){ //need the user to be able to login
						next(true);
					}else{
						thisNode.sendEvent(tunnel.remoteId, 'Unauthorised', {
							message: 'You do not have permission to send messages to this server.'
						});
					}
				}else{
					thisNode.Session_Get(tunnel.remoteId, 'GroupSecurity', function(err, val){
						thisNode.GroupSecurity_GetUserPermission({
							user: val.authorised?val.user.id:false,
							topic: topic,
							message: message
			...
			thisNode.on('Tunnel.Ready', function(destination, tunnel){
				//start a session
				thisNode.Session_Start(destination, function(){
					thisNode.Session_Set(destination, 'GroupSecurity', {
						authorised: false
					},
					function(){
			...
			thisNode.on('GroupGroupSecurity.AddGroup', function(message, rawMessage){
			...
			thisNode.on('GroupSecurity.SendMessage', function(message, rawMessage){
				thisNode.GroupSecurity_SendMessage(message, rawMessage);
			});
			
			//Process any supplied configuration options
			if(cfg){
				if(cfg.buildStructure===true){
					new thisNode.StorageManager.Model('groupsecurity_user',{
						fields:{
							username: {
								validators: {
									string: {}
					...
					}, function(usrErr, userModel){
						new thisNode.StorageManager.Model('groupsecurity_group', {
							fields:{
								displayName:{
									validators: {
										string:{}
							...
							}, function(grpChanErr, groupChannelModel){
								new thisNode.StorageManager.Channel({
									name: 'users',
									model: groupModel.model
								}, function(grpChanErr, userChannelModel){
				...
				if(cfg.unprotectedTopics){
					for(var i=0;i<cfg.unprotectedTopics.length;i++){
						thisNode.setDataValueByString(thisNode.GroupSecurity_Settings.unprotectedTopics, cfg.unprotectedTopics[i], true);	
					}
				}
			}else{
			...
			thisNode.StorageManager.find(message, {}, thisNode.GroupSecurity_Settings.store, thisNode.GroupSecurity_Settings.userChannel, function(err, recs){
				if(!err){
					if(recs.length>0){
						...
						thisNode.Session_Set(rawMessage._message.sender, 'GroupSecurity', 
							{
								authorised: true,
								user: recs[0].record
							}, 
							function(){
								...
								thisNode.emit('GroupSecurity.UserLoggedIn', {
									user: recs[0],
									remoteNode: rawMessage._message.sender
								});			
							}
						);
		...
		GroupSecurity_Logout: function(message, rawMessage){
			...
			thisNode.sendEvent(rawMessage._message.sender, 'GroupSecurity.LogoutResponse', {}, rawMessage._message.id);
		...
		GroupSecurity_SendMessage: function(message, rawMessage){
			var thisNode = this;
			console.log('SENDING MESSAGE');
			console.log(message);
			thisNode.sendEvent(message.destination, message.topic, message.message);
		},
		...
		GroupSecurity_GetUserPermission: function(message, rawMessage){
			...
			if(topic=='init'){ //init MUST be allowed
				...
				if(!userId){ //only hope is if the event is in the unprotected list
					...
					console.log(topic);
					if(thisNode.getDataValueByString(thisNode.GroupSecurity_Settings.unprotectedTopics, topic)==true){
						allowed = true;
						permission = true;	
					}
