var mixinFunctions = {
	...
	Tickers_createTicker: function(name, interval, options, callback){
		...
		if(!err){
			...
			thisNode.on('Tickers.'+name+'.Start', thisNode.onStartByName);
			thisNode.on('Tickers.'+name+'.Stop', thisNode.onStopByName);
			thisNode.on('Tickers.'+name+'.Remove', thisNode.onRemoveByName);
	...
	Tickers_startTicker: function(options, rawMessage, callback){
		var thisNode = this;
