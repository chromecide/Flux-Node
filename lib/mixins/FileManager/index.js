...
var mixinFunctions = {
		    	...
		    	if(curr.mtime.getTime()>prev.mtime.getTime()){
					if (curr.isDirectory() && prev.isDirectory()) {
						...
						if (curr.nlink > prev.nlink) { //files were added
		        			...
		        			if(watcher.FileList){
		        				fs.readdir(filePath, function(err, files) {
		        					...
		        					_.each(addedFiles, function(filename){
		        						//create a timeout for checking if a file has finished growing
		        						thisNode.FileManager_monitorGrowth(filePath+'/'+filename, function(){
		        							thisNode.emit(
		        			...
		        			var watcher = thisNode.getSetting('FileManager.Watchers.'+id);
		        			fs.readdir(filePath, function(err, files) {
	        					var deletedFiles = _.difference(watcher.FileList, files);
	        					
	        					_.each(deletedFiles, function(filename){
	        						thisNode.emit(
	...
	FileManager_monitorGrowth: function(filePath, callback){
		var startStat = fs.statSync(filePath);
		
		var monitorFunc = function(pathtofile, lastSize, cbk){
			...
			if(stat.size==lastSize){
				...
				setTimeout(monitorFunc, 1000, pathtofile, stat.size, cbk);
			}
		}
		
		setTimeout(monitorFunc, 3000, filePath, startStat.size, callback);
	}
}
