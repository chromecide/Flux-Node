var mixinFunctions = {
	init: function(cfg, callback){
		...
		thisNode.addSetting('RuleManager.Store', false, {boolean:{}});
							...
							for(var cIdx=0; cIdx<criteria.length;cIdx++){
								switch(typeof criteria[cIdx]){
									case 'function':
										...
										break;
									case 'object':
										console.log('NEED TO VALIDATE CRITERIA');
										break;
								}
							}
											...
											case 'string':
												switch(action){
													case 'emit':
														...
														break;
													case 'Respond':
														thisNode.RuleManager_doRespondAction(actionMap, message, rawMessage);
														break;
												}
												break;
											case 'function':
	...
	RuleManager_doRespondAction: function(map, message, rawMessage, callback){
		var thisNode = this;
		if(rawMessage && rawMessage._message.sender && rawMessage._message.sender!=thisNode.id){
			thisNode.RuleManager_doMapObject(map, message, rawMessage, function(err, mappedValue){
				thisNode.sendEvent(rawMessage._message.sender, mappedValue.topic, mappedValue.message, rawMessage._message.id);
				if(callback){
					callback(false);
				}
			});	
	...
	RuleManager_GenerateMapData: function(){
		var data = {};
		//Date information
		data.Date = {
			Now: new Date(),
